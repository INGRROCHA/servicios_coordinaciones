<div> 
                ...
                @if($secciones && $secciones->count() > 0)
                    @foreach($secciones as $seccion)
                        <button wire:click="setSeccionFilter({{ $seccion->id_seccion }})" 
                                class="px-3 py-1.5 text-xs font-bold rounded-lg border transition shadow-sm 
                                {{ $seccionId == $seccion->id_seccion ? 'bg-teal-600 text-white border-teal-600' : 'bg-teal-55 text-teal-800 border-teal-200 hover:bg-teal-100' }}">
                            {{ $seccion->seccion }}
                        </button>
                    @endforeach
                @endif
            ...
            <div class="inline-block align-bottom bg-gray-200 rounded-lg text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
                ...
                <div class="p-6 overflow-y-auto max-h-[80vh] flex justify-center bg-gray-200">
                    <div class="pdf-preview shadow-xl">
                        <style>
                            .pdf-preview { background-color: white; width: 100%; max-width: 800px; margin: 0 auto; padding: 30px; font-family: sans-serif; color: black; }
                            .pdf-preview h1, .pdf-preview h2, .pdf-preview h3, .pdf-preview h4, .pdf-preview h5, .pdf-preview h6, .pdf-preview p, .pdf-preview span, .pdf-preview label { font-family: sans-serif; }
                            .pdf-preview table { width: 100%; border-collapse: collapse; margin-bottom: 15px !important; }
                            .pdf-preview table thead th { height: 28px; text-align: left; font-size: 16px; }
                            .pdf-preview table, .pdf-preview th, .pdf-preview td { border: 1px solid #ddd; padding: 8px; font-size: 14px; }
                            .pdf-preview .text-start { text-align: left; }
                            .pdf-preview .text-end { text-align: right; }
                            .pdf-preview .text-center { text-align: center; }
                            .pdf-preview .company-data span { margin-bottom: 4px; display: inline-block; font-size: 14px; font-weight: 400; }
                        ...
                        <table class="order-details" style="margin-top: 10px;">
                            <tr>
                                <th width="33%" class="text-end company-data no-border"><span>Sección: {{ $ticketSeleccionado->seccion->seccion ?? 'N/A' }}</span></th>
                                <th width="33%" class="text-center company-data no-border"><span>Ticket #: {{ $ticketSeleccionado->id_ticket }}</span></th>
                                <th width="33%" class="text-end company-data no-border"><span>Fecha: {{ date('d/m/y') }}</span></th>   
                            </tr>
                        </table>
                        <table class="user-details" style="margin-top: 15px;">
                            <tbody>
                                <tr><td width="30%">Nombre del solicitante</td><td width="70%">{{ $ticketSeleccionado->nombre ?? 'Sin dato' }}</td></tr>
                                <tr><td>División/Coordinación General</td><td>{{ $ticketSeleccionado->adscripcion ?? 'Sin dato' }}</td></tr>
                                <tr><td>Departamento/Coordinación Administrativa</td><td>{{ $ticketSeleccionado->dpto_coord ?? 'Sin dato' }}</td></tr>
                                <tr><td>Área Académica/Sección Administrativa</td><td>{{ $ticketSeleccionado->area_secc ?? 'Sin dato' }}</td></tr>
                            </tbody>
                        </table>
                        <table>
                            <tbody>
                                <tr>
                                    <td width="10%">Edificio</td><td width="40%">{{ $ticketSeleccionado->dpersonales->edificio ?? 'Sin dato' }}</td>
                                    <td width="10%">Nivel</td><td width="40%">{{ $ticketSeleccionado->dpersonales->nivel ?? 'Sin dato' }}</td>
                                </tr>
                                <tr>
                                    <td>Cubículo</td><td>{{ $ticketSeleccionado->dpersonales->cubiculo ?? 'Sin dato' }}</td>
                                    <td>Extensión</td><td>{{ $ticketSeleccionado->dpersonales->extension ?? 'Sin dato' }}</td>
                                </tr>
                            </tbody>
                        </table>
                        <table class="servicio-details">
                            <thead><tr><th class="no-border" colspan="2" style="background:#f3f4f6;">Descripción del Trabajo a Realizar</th></tr></thead>
                            <tbody>
                                <tr><td width="20%">Tipo de Trabajo</td><td width="80%">{{ $ticketSeleccionado->servicio->servicio ?? 'N/A' }}</td></tr>
                                <tr><td>Descripción</td><td>{{ $ticketSeleccionado->descripcion }}</td></tr>
                            </tbody>
                        </table>
                        <table>
                            <thead><tr><th class="no-border" colspan="1" style="background:#f3f4f6;">Observaciones</th></tr></thead>
                            <tbody><tr><td>{{ $ticketSeleccionado->observaciones ?? 'Sin dato' }}</td></tr></tbody>
                        </table>    
                        <table style="margin-top: 20px;">
                            <tbody>
                                <tr><td>Recibí de conformidad al terminar el servicio</td><td class="text-center"><br><br>__________________________<br>Nombre y Firma</td></tr>
                                <tr><td>Responsable del Área</td><td><br>{{ session('nombre_completo') ?? 'Sin dato' }}</td></tr>
                                <tr><td>Trabajador que realizó el servicio</td><td><br>{{ $ticketSeleccionado->trabajadores->nombre ?? 'N/A'}}</td></tr>
                                <tr><td>Fecha y hora de terminación</td><td class="text-center"> </td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
                <div class="bg-gray-50 px-4 py-3 flex flex-col sm:flex-row justify-between items-center rounded-b-lg border-t gap-3 sm:gap-0">
                    <button wire:click="cerrarModalPdf" type="button" class="w-full sm:w-auto px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 transition shadow">Cerrar Ventana</button>
    ...
    function imprimirTicketPDF() {
        ...
        ventanaImpresion.document.write('<html><head><title>Imprimir Ticket</title><style>body { margin: 10px; padding: 10px; font-family: sans-serif; color: black; } table { width: 100%; border-collapse: collapse; margin-bottom: 15px !important; } th { height: 28px; text-align: left; font-size: 16px; } table, th, td { border: 1px solid #ddd; padding: 8px; font-size: 14px; } .text-start { text-align: left; } .text-end { text-align: right; } .text-center { text-align: center; } .company-data span { margin-bottom: 4px; display: inline-block; font-size: 14px; font-weight: 400; } .no-border { border: 1px solid #fff !important; } @media print { @page { margin: 1cm; } }</style></head><body>');
        ventanaImpresion.document.write(contenidoTicket);
        ventanaImpresion.document.write('</body></html>');
        ventanaImpresion.document.close();
        ventanaImpresion.focus();
        setTimeout(function() { ventanaImpresion.print(); ventanaImpresion.close(); }, 300);
    }
    </script>
</div> 